...
int calib::BundleAdjustment::refine_BA(std::vector<int>& index, calib::Imagefeature& current_feature, std::vector<calib::Imagefeature>& features,
				...
				calib::CameraParams& current_para)     //�������ŵ��Ǻ�ƴ��ͼ��������ͼ����
{
	double cameraParam[99*4];    //������������������������������������������������������
	cv::SVD svd2;
	for (size_t i = 0; i < cameras.size(); i++) {
	...
	// parameters for cameras and points are added automatically.
	ceres::Problem problem;
	for (size_t edge_idx = 0; edge_idx < connectionNum; edge_idx++) {       //��ÿ��ͼ����д���
		/*if (current_matchesInfo[edge_idx].confidence < 1.5)
			continue;*/
		if (current_matchesInfo[edge_idx].confidence < 1)   //2.9����ͼ��֮���������ƥ������õ���ͼ��֮������Ŷ�
			continue;
		const calib::Imagefeature& features1 = current_feature;
		const calib::Imagefeature& features2 = features[index[edge_idx]];
		const calib::Matchesinfo& matches_info = current_matchesInfo[edge_idx];      //�ڼ��������ε�ͼ���
		double focal2 = cameraParam[index[edge_idx] * 4 + 3];
		for (size_t k = 0; k < matches_info.matches.size(); ++k) {     //��ÿ����������д���
			if (!matches_info.inliers_mask[k])    //�����Բ�������
				continue;
			const cv::DMatch& m = matches_info.matches[k];
			cv::Point2f p1 = features1.keypt[m.queryIdx].pt;
			cv::Point2f p2 = features2.keypt[m.trainIdx].pt;
	...
	options.minimizer_progress_to_stdout = true;
	options.jacobi_scaling = false;
	options.max_num_iterations = 2000;   //2000
	options.function_tolerance = 1e-8;
	options.parameter_tolerance = 1e-8;
	ceres::Solver::Summary summary;
	ceres::Solve(options, &problem, &summary);
	...
	Rvec_[1] = current[1];
	Rvec_[2] = current[2];
	ceres::AngleAxisToRotationMatrix(Rvec_, R_1);      //����ת����ת��Ϊ��ת������������ƴ��
