...
int read_para(vector<calib::CameraParams> &cameras, vector<Point> &corners, vector<Size>&sizes)
{
	...
	for (int i = 0; i < Row*Col; i++)   //����û���Զ�����ͼƬ����������������
	{
		getline(para, str, ' ');
...
int GetCurrentPara(vector<calib::CameraParams>& cameras, cv::Point2f current_point, calib::CameraParams &current_para)
{
	...
	///////////////����������Ҫ�ӵ�readpara�������
	vector<double>value;
	vector<float>mapx(1, (current_point.x - X_MIN)*1.0 / dX);    //Ĭ�������00��ʼ
	vector<float>mapy(1, (current_point.y - Y_MIN)*1.0 / dY);    //Ĭ�������00��ʼ
	remap(focals, value, mapx, mapy, INTER_LINEAR);    //������Ҫclearһ�£���ʵ�鷵��ֵ��push���Ǹ���
...
int warp(std::vector<cv::Mat>& imgs)
{
	...
	{
		match.current_feature_thread_(src, imgs[index[i]], features[i], current_feature, current_matchesInfo[i],i);    //������ƴ��ͼ�����Χͼ�������ƥ��
	}
	//calib::BundleAdjustment bundleAdjust;
	calib::BundleAdjustment bundleAdjust;
	bundleAdjust.refine_BA(index, current_feature, features, current_matchesInfo, cameras, current_para);    //�õ���ǰcurrent_camera��׼ȷֵ
